...
def write_json(data):
    ...
    file_name = '/media/ukrainer/G/site/site' + str(name) + '.txt'
...
def get_list_files():
    my_txt = []
    for root, dirs, files in os.walk('/media/ukrainer/G/site'):
        my_txt.extend([join(root, file) for file in files if file.endswith('txt')])
        dirs.clear()
    return my_txt
